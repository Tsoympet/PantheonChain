...
public:
     ...
     */
    virtual std::optional<std::vector<uint8_t>> GetPublicKey(
     ...
     */
    virtual std::optional<std::string> GetAddress(
        const DerivationPath& path,
     ...
     */
    virtual std::optional<primitives::Transaction> SignTransaction(
        const primitives::Transaction& tx,
        const std::vector<DerivationPath>& input_paths
    ) = 0;
