...
VerificationResult FirmwareVerifier::VerifyFirmware(
    ...
    if (firmware_info_opt) {
        ...
        // Verify signature
        bool sig_valid = VerifySignature(
            device_firmware,
            result.firmware_info.signature,
            vendor
        );
        ...
        result.security_advisories = GetSecurityAdvisories(
            vendor,
            result.firmware_info.version
        );
...
bool FirmwareVerifier::VerifyHash(
...
bool FirmwareVerifier::VerifySignature(
...
std::optional<std::string> FirmwareVerifier::CheckLatestVersion(
    const std::string& vendor,
...
std::vector<std::string> FirmwareVerifier::GetSecurityAdvisories(
    const std::string& vendor,
    const std::string& version) {
...
std::optional<FirmwareInfo> FirmwareVerifier::GetFirmwareInfo(
    const std::vector<uint8_t>& firmware_hash) {
...
bool FirmwareVerifier::VerifySchnorrSignature(
...
std::optional<FirmwareInfo> FirmwareUpdateManager::CheckForUpdates(
    const std::string& vendor,
    const std::string& current_version) {
...
std::optional<std::vector<uint8_t>> FirmwareUpdateManager::DownloadFirmware(
...
VerificationResult FirmwareUpdateManager::VerifyUpdate(
    const std::vector<uint8_t>& firmware,
    const std::string& vendor) {
...
bool FirmwareUpdateManager::InstallUpdate(
    [[maybe_unused]] const std::vector<uint8_t>& device_id,
...
std::optional<std::string> BootloaderVerifier::CheckBootloaderVersion(
    [[maybe_unused]] const std::string& vendor,
    [[maybe_unused]] const std::string& version) {
...
VerificationStatus SupplyChainVerifier::VerifyDeviceSeals(
    [[maybe_unused]] const std::string& device_serial,
    [[maybe_unused]] const std::string& vendor) {
...
bool SupplyChainVerifier::CheckDeviceRegistry(
    [[maybe_unused]] const std::string& device_serial,
    [[maybe_unused]] const std::string& vendor) {
