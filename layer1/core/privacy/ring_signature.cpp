...
std::array<uint8_t, 32> RingSigner::GenerateKeyImage(
    const std::array<uint8_t, 32>& secret_key,
...
std::array<uint8_t, 32> RingSigner::HashToPoint(
    const std::array<uint8_t, 33>& public_key)
{
...
bool LinkableRingSignature::VerifyLinkable(
    const RingSignature& signature,
    const std::vector<uint8_t>& message)
{
...
bool LinkableRingSignature::AreLinked(
...
std::array<uint8_t, 33> StealthAddress::Generate(
...
std::array<uint8_t, 32> StealthAddress::RecoverSecretKey(
    const std::array<uint8_t, 32>& view_secret,
