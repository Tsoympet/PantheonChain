...
void TransferCircuit::SetWitness(
    ...
    const std::vector<uint8_t>& randomness)
{
    ...
    // Pack witness data
    witness_data_.insert(witness_data_.end(), 
                        sender_secret.begin(), 
                        sender_secret.end());
...
void TransferCircuit::SetPublicInputs(
...
ZKProver::ZKProver(const ProofParameters& params)
    : params_(params)
{
}
...
std::optional<ZKProof> ZKProver::GenerateProof(
    Circuit& circuit,
...
ZKVerifier::ZKVerifier(const ProofParameters& params)
    : params_(params)
{
}
...
std::array<uint8_t, 32> PedersenCommitment::Commit(
    uint64_t value,
...
std::array<uint8_t, 32> Nullifier::Generate(
