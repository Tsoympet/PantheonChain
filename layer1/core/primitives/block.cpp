...
std::optional<Block> Block::Deserialize(const uint8_t* data, size_t len) {
    ...
    if (tx_count == 0 || tx_count > 1000000) return std::nullopt; // Sanity check
...
std::array<uint8_t, 32> MerkleTree::CalculateRoot(const std::vector<std::array<uint8_t, 32>>& hashes) {
    if (hashes.empty()) {
