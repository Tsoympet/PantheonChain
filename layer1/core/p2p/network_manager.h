...
public:
    ...
    void SetOnVerack(std::function<void()> callback) {
        on_verack_ = callback;
    }
    void SetOnPing(std::function<void(uint64_t)> callback) {
        on_ping_ = callback;
    }
    void SetOnPong(std::function<void(uint64_t)> callback) {
        on_pong_ = callback;
    }
    void SetOnInv(std::function<void(const InvMessage&)> callback) {
        on_inv_ = callback;
    }
    ...
    void SetOnAddr(std::function<void(const AddrMessage&)> callback) {
        on_addr_ = callback;
    }
...
public:
    ...
    void SetOnNewPeer(std::function<void(const std::string&)> callback) {
        on_new_peer_ = callback;
    }
    void SetOnBlock(std::function<void(const std::string&, const primitives::Block&)> callback) {
        on_block_ = callback;
    }
    void SetOnTransaction(std::function<void(const std::string&, const primitives::Transaction&)> callback) {
    ...
    void SetOnGetHeaders(std::function<void(const std::string&, const GetHeadersMessage&)> callback) {
        on_getheaders_ = callback;
    }
