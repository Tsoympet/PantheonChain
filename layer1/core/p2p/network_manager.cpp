...
PeerConnection::PeerConnection(int socket_fd, const std::string& address, uint16_t port)
    : socket_fd_(socket_fd), address_(address), port_(port),
      state_(PeerState::CONNECTING), version_(0), height_(0),
      services_(0), nonce_(0), network_magic_(NetworkMagic::MAINNET) {
...
void PeerConnection::ProcessMessage(const MessageHeader& header, const uint8_t* payload, size_t len) {
...
NetworkManager::NetworkManager(uint16_t listen_port, uint32_t network_magic)
    : listen_port_(listen_port), network_magic_(network_magic),
      running_(false), listen_socket_(-1) {
}
...
void NetworkManager::HandlePeer(const std::string& peer_id) {
    ...
    peer->SetOnPing([&peer](uint64_t nonce) {
        peer->SendPong(nonce);
    });
...
void NetworkManager::RequestBlocks(const std::string& peer_id, uint32_t start_height, uint32_t count) {
    std::lock_guard<std::mutex> lock(peers_mutex_);
    auto it = peers_.find(peer_id);
