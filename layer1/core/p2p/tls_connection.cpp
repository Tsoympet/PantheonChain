...
bool TLSConnection::InitializeGlobalContext() {
    ...
    SSL_CTX_set_options(server_ctx_, SSL_OP_NO_SSLv2 | SSL_OP_NO_SSLv3 | SSL_OP_NO_TLSv1 | SSL_OP_NO_TLSv1_1);
    SSL_CTX_set_options(client_ctx_, SSL_OP_NO_SSLv2 | SSL_OP_NO_SSLv3 | SSL_OP_NO_TLSv1 | SSL_OP_NO_TLSv1_1);
...
bool TLSConnection::LoadCertificate(const std::string& cert_file, const std::string& key_file) {
    if (!initialized_ || !server_ctx_) return false;
...
bool TLSConnection::LoadTrustedCAs(const std::string& ca_file) {
    if (!initialized_ || !client_ctx_) return false;
