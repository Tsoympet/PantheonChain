...
void CertificateRotation::SetRotationCallback(std::function<void(const CertificateInfo&)> callback) {
    callback_ = callback;
}
...
void CertificateRotation::Start() {
    ...
    std::thread rotation_thread([this]() {
        this->RotationLoop();
    });
    rotation_thread.detach();
    
    std::cout << "Certificate rotation checker started (interval: " 
              << check_interval_ << "s)\n";
}
...
bool CertificateRotation::LoadCertificate(const std::string& cert_path, const std::string& key_path) {
    FILE* fp = fopen(cert_path.c_str(), "r");
    if (!fp) {
        return false;
    }
