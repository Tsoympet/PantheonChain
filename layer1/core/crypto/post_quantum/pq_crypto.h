...
class DilithiumSignature {
  public:
     ...
     * Sign message
     */
    static Signature Sign(const std::vector<uint8_t>& message, const SecretKey& secret_key);

    /**
     * Verify signature
     */
    static bool Verify(const std::vector<uint8_t>& message, const Signature& signature,
...
class KyberKEM {
  public:
     ...
     */
    static bool Encapsulate(const PublicKey& public_key, Ciphertext& ciphertext,
                            SharedSecret& shared_secret);

    /**
     * Decapsulate shared secret
     */
    static std::optional<SharedSecret> Decapsulate(const Ciphertext& ciphertext,
...
class HybridCrypto {
  public:
     ...
     */
    static bool GenerateKeyPair(HybridPublicKey& public_key, HybridSecretKey& secret_key);

    /**
     * Sign with both classical and PQ
     */
    static HybridSignature Sign(const std::vector<uint8_t>& message,
                                const HybridSecretKey& secret_key);

    /**
     * Verify both signatures (both must be valid)
     */
    static bool Verify(const std::vector<uint8_t>& message, const HybridSignature& signature,
...
class PQAddress {
  public:
     ...
     */
    static std::string
    FromPublicKey(const std::array<uint8_t, DilithiumSignature::PUBLIC_KEY_SIZE>& public_key);
     ...
     */
    static std::optional<std::array<uint8_t, DilithiumSignature::PUBLIC_KEY_SIZE>>
    ToPublicKey(const std::string& address);
};
