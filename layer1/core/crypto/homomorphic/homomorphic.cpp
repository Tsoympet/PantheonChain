...
BFVEncryption::BFVEncryption() 
    : plain_modulus_(1024), coeff_modulus_(1ULL << 60) {}
...
Ciphertext BFVEncryption::Encrypt(uint64_t plaintext, [[maybe_unused]] const PublicKey& public_key) {
    Ciphertext ct;
...
uint64_t BFVEncryption::Decrypt(const Ciphertext& ciphertext, [[maybe_unused]] const SecretKey& secret_key) {
...
double CKKSEncryption::Decrypt(const Ciphertext& ciphertext, [[maybe_unused]] const SecretKey& secret_key) {
    if (ciphertext.coefficients.empty()) {
        return 0.0;
    }
...
Ciphertext HomomorphicCompute::Compare([[maybe_unused]] const Ciphertext& a, [[maybe_unused]] const Ciphertext& b) {
...
Ciphertext HomomorphicCompute::EvaluatePolynomial(
    const Ciphertext& x,
