...
uint32_t ShardIdentifier::GetShardForAddress(const std::vector<uint8_t>& address,
...
uint32_t ShardIdentifier::GetShardForTransaction(const std::array<uint8_t, 32>& tx_hash,
                                                 uint32_t total_shards) {
    if (total_shards == 0 || total_shards == 1) {
        return 0;
    }
...
bool ShardIdentifier::BelongsToShard(const std::vector<uint8_t>& address, uint32_t shard_id,
                                     uint32_t total_shards) {
    return GetShardForAddress(address, total_shards) == shard_id;
}

// ShardStateManager Implementation
ShardStateManager::ShardStateManager(const ShardConfig& config) : config_(config) {}

ShardStateManager::~ShardStateManager() = default;

bool ShardStateManager::OwnsAddress(const std::vector<uint8_t>& address) const {
    return ShardIdentifier::BelongsToShard(address, config_.shard_id, config_.total_shards);
}
...
bool ShardStateManager::ValidateCrossShardProof(const CrossShardTx& tx) const {
    ...
    if (tx.source_shard >= config_.total_shards || tx.destination_shard >= config_.total_shards) {
        return false;
    }
...
ShardCoordinator::ShardCoordinator(uint32_t total_shards) : total_shards_(total_shards) {}

ShardCoordinator::~ShardCoordinator() = default;

bool ShardCoordinator::RegisterShard(uint32_t shard_id,
