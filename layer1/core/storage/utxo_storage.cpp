...
bool UTXOStorage::AddUTXO(const std::array<uint8_t, 32>& txid, uint32_t vout, const primitives::TxOutput& output) {
    if (!db_) {
        return false;
    }
...
std::optional<primitives::TxOutput> UTXOStorage::GetUTXO(const std::array<uint8_t, 32>& txid, uint32_t vout) {
    if (!db_) {
        return std::nullopt;
    }
