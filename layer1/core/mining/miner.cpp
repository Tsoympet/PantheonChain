...
Miner::Miner(chainstate::ChainState& chainstate, const std::vector<uint8_t>& coinbase_pubkey)
    : chainstate_(chainstate), coinbase_pubkey_(coinbase_pubkey),
      is_mining_(false), hashrate_(0), total_hashes_(0) {
}
...
std::optional<BlockTemplate> Miner::CreateBlockTemplate(size_t max_transactions) {
    ...
    coinbase_rewards.push_back(primitives::AssetAmount(
        ...
        tal_reward + fees[primitives::AssetID::TALANTON]
    ));
    coinbase_rewards.push_back(primitives::AssetAmount(
        primitives::AssetID::DRACHMA,
        dra_reward + fees[primitives::AssetID::DRACHMA]
    ));
    coinbase_rewards.push_back(primitives::AssetAmount(
        primitives::AssetID::OBOLOS,
        obl_reward + fees[primitives::AssetID::OBOLOS]
    ));
    ...
    block_template.block.transactions.insert(
        block_template.block.transactions.end(),
        transactions.begin(),
        transactions.end()
    );
...
std::optional<primitives::Block> Miner::MineBlock(
...
primitives::Transaction Miner::CreateCoinbaseTransaction(
    uint32_t height,
...
std::map<primitives::AssetID, uint64_t> Miner::CalculateFees(
...
std::array<uint8_t, 32> Miner::ComputeMerkleRoot(
    const std::vector<primitives::Transaction>& transactions
) {
