...
TransactionValidator::ValidateStructure(const primitives::Transaction& tx) {
    // Coinbase can have zero inputs
    if (!tx.IsCoinbase()) {
        if (tx.inputs.empty()) {
            return ValidationError(ValidationError::Type::TX_NO_INPUTS,
                                   "Transaction has no inputs");
        }
    }

    if (tx.outputs.empty()) {
        return ValidationError(ValidationError::Type::TX_NO_OUTPUTS, "Transaction has no outputs");
    }
    ...
    for (const auto& input : tx.inputs) {
        if (seen_inputs.count(input.prevout) > 0) {
            return ValidationError(ValidationError::Type::TX_DUPLICATE_INPUTS,
    ...
    for (const auto& output : tx.outputs) {
        if (!output.value.IsValid()) {
            return ValidationError(ValidationError::Type::TX_INVALID_AMOUNT,
...
TransactionValidator::ValidateAgainstUTXO(const primitives::Transaction& tx,
    ...
    for (const auto& input : tx.inputs) {
        ...
        if (!coin) {
            return ValidationError(ValidationError::Type::TX_MISSING_INPUT,
                                   "Transaction input does not exist in UTXO set");
        }

        // Check coinbase maturity
        if (!coin->IsSpendable(height)) {
            return ValidationError(ValidationError::Type::TX_IMMATURE_COINBASE,
...
std::optional<ValidationError>
TransactionValidator::ValidateSignatures(const primitives::Transaction& tx,
    ...
    for (size_t i = 0; i < tx.inputs.size(); i++) {
        ...
        if (coin->output.pubkey_script.size() != crypto::Schnorr::PUBLIC_KEY_SIZE) {
            return ValidationError(ValidationError::Type::TX_INVALID_SIGNATURE,
                                   "Invalid pubkey script size in output being spent");
        }

        crypto::Schnorr::PublicKey pubkey;
        std::copy(coin->output.pubkey_script.begin(), coin->output.pubkey_script.end(),
                  pubkey.begin());
        ...
        std::copy(input.signature_script.begin(), input.signature_script.end(), signature.begin());
...
std::optional<ValidationError> BlockValidator::ValidateStructure(const primitives::Block& block) {
    // Must have at least one transaction
    if (block.transactions.empty()) {
        return ValidationError(ValidationError::Type::BLOCK_NO_TRANSACTIONS,
                               "Block has no transactions");
    }

    // First transaction must be coinbase
    if (!block.transactions[0].IsCoinbase()) {
        return ValidationError(ValidationError::Type::BLOCK_NO_COINBASE,
    ...
    for (size_t i = 1; i < block.transactions.size(); i++) {
        if (block.transactions[i].IsCoinbase()) {
            return ValidationError(ValidationError::Type::BLOCK_MULTIPLE_COINBASE,
    ...
    if (calculated_root != block.header.merkle_root) {
        return ValidationError(ValidationError::Type::BLOCK_INVALID_MERKLE_ROOT,
...
std::optional<ValidationError> BlockValidator::ValidatePoW(const primitives::Block& block) {
    auto hash = block.GetHash();
    if (!consensus::Difficulty::CheckProofOfWork(hash, block.header.bits)) {
        return ValidationError(ValidationError::Type::BLOCK_INVALID_POW,
...
std::optional<ValidationError>
BlockValidator::ValidateCoinbase(const primitives::Block& block, uint32_t height,
    ...
    for (const auto& [asset, amount] : coinbase_amounts) {
        ...
        if (!consensus::Issuance::IsValidBlockReward(height, asset, amount)) {
            return ValidationError(ValidationError::Type::BLOCK_INVALID_COINBASE_REWARD,
        ...
        if (supply + amount < supply) {
            return ValidationError(ValidationError::Type::BLOCK_EXCEEDS_SUPPLY_CAP,
...
std::optional<ValidationError>
BlockValidator::ValidateBlock(const primitives::Block& block, const chainstate::UTXOSet& utxo_set,
                              uint32_t height,
                              const std::map<primitives::AssetID, uint64_t>& current_supply) {
    // Validate structure
    ...
    for (size_t i = 1; i < block.transactions.size(); i++) {
        ...
        if (error) {
            return ValidationError(ValidationError::Type::BLOCK_INVALID_TRANSACTION,
