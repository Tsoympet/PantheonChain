...
public:
     ...
     * - Valid amounts
     */
    static std::optional<ValidationError> ValidateStructure(
     ...
     */
    static std::optional<ValidationError> ValidateAgainstUTXO(
        const primitives::Transaction& tx,
...
public:
     ...
     */
    static std::optional<ValidationError> ValidatePoW(
        const primitives::Block& block
    );
     ...
     */
    static std::optional<ValidationError> ValidateCoinbase(
        const primitives::Block& block,
        uint32_t height,
     ...
     */
    static std::optional<ValidationError> ValidateBlock(
        const primitives::Block& block,
        const chainstate::UTXOSet& utxo_set,
        uint32_t height,
        const std::map<primitives::AssetID, uint64_t>& current_supply
