...
struct MempoolEntry {
    ...
    MempoolEntry(const primitives::Transaction& t, uint64_t f, uint32_t tm, uint32_t h, bool rbf = false)
...
public:
     ...
     */
    std::vector<std::vector<primitives::Transaction>> GetTransactionPackages(size_t max_count) const;
     ...
     */
    std::vector<std::array<uint8_t, 32>> GetDescendants(
        const std::array<uint8_t, 32>& txid
    ) const;
...
private:
    ...
    // RBF constants
    static constexpr uint64_t MIN_RBF_FEE_INCREMENT = 1000; // Minimum fee increase for RBF (satoshis)
     ...
     */
    std::vector<std::array<uint8_t, 32>> GetConflictingTransactions(
        const primitives::Transaction& tx
    ) const;
     ...
     */
    uint64_t CalculateReplacedFees(
