...
AggregatedSignature AggregatedSignature::Deserialize(const std::vector<uint8_t>& data, size_t& pos) {
    if (pos + 1 > data.size()) {
...
bool MultisigValidator::VerifySchnorrSignature(
    const PubKey& pubkey,
    const Signature& sig,
...
bool MultisigValidator::VerifySignatures(
