...
class MultisigValidator {
  public:
    ...
    static bool VerifySignatures(const MultisigPolicy& policy, const AggregatedSignature& agg_sig,
                                 const std::vector<uint8_t>& message);

    // Verify a single Schnorr signature
    static bool VerifySchnorrSignature(const PubKey& pubkey, const Signature& sig,
                                       const std::vector<uint8_t>& message);
