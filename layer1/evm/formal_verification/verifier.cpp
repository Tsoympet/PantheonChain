...
VerificationResult ContractVerifier::VerifyContract(
...
std::vector<std::string> SymbolicExecutor::FindAssertionViolations(
    const std::vector<uint8_t>& bytecode)
{
...
UpgradeableContract::Proxy UpgradeableContract::CreateProxy(
    const std::vector<uint8_t>& implementation,
...
bool UpgradeableContract::UpgradeImplementation(
    Proxy& proxy,
...
bool UpgradeableContract::VerifyUpgradeSafety(
    const std::vector<uint8_t>& old_bytecode,
...
bool UpgradeableContract::TransferAdmin(
    Proxy& proxy,
    const std::vector<uint8_t>& new_admin,
...
std::vector<StorageLayoutAnalyzer::StorageSlot> StorageLayoutAnalyzer::AnalyzeLayout(
    const std::vector<uint8_t>& bytecode)
{
...
bool StorageLayoutAnalyzer::AreLayoutsCompatible(
    const std::vector<StorageSlot>& old_layout,
...
std::vector<std::string> StorageLayoutAnalyzer::DetectCollisions(
    const std::vector<StorageSlot>& layout)
{
    ...
    for (const auto& [slot, count] : slot_counts) {
        if (count > 1) {
            collisions.push_back("Slot " + std::to_string(slot) + " used " + 
