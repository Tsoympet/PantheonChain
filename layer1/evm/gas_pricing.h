 ...
 */
inline uint64_t CalculateNextBaseFee(uint64_t parent_base_fee, uint64_t parent_gas_used,
    ...
    if (parent_gas_used > target_gas) {
        ...
        if (parent_base_fee <= UINT64_MAX / gas_delta) {
            base_fee_delta = (parent_base_fee * gas_delta) / target_gas /
                             GasPricing::BASE_FEE_CHANGE_DENOMINATOR;
        } else {
            // Do division first to prevent overflow
            base_fee_delta = (parent_base_fee / GasPricing::BASE_FEE_CHANGE_DENOMINATOR) *
                             gas_delta / target_gas;
        }
    ...
    if (parent_base_fee <= UINT64_MAX / gas_delta) {
        base_fee_delta =
            (parent_base_fee * gas_delta) / target_gas / GasPricing::BASE_FEE_CHANGE_DENOMINATOR;
    } else {
        base_fee_delta =
            (parent_base_fee / GasPricing::BASE_FEE_CHANGE_DENOMINATOR) * gas_delta / target_gas;
    }
 ...
 */
inline uint64_t CalculateEffectiveGasPrice(uint64_t base_fee, uint64_t max_fee_per_gas,
 ...
 */
inline bool ValidateTransactionFees(uint64_t base_fee, uint64_t max_fee_per_gas,
                                    uint64_t max_priority_fee_per_gas) {
...
inline uint64_t CalculateTransactionFee(uint64_t gas_used, uint64_t base_fee,
                                        uint64_t max_fee_per_gas, uint64_t max_priority_fee_per_gas,
                                        uint64_t& base_fee_burned, uint64_t& priority_fee_paid) {
    uint64_t effective_gas_price =
        CalculateEffectiveGasPrice(base_fee, max_fee_per_gas, max_priority_fee_per_gas);
