...
private:
    ...
    // Helper functions
    NodePtr Insert(NodePtr node, const std::vector<uint8_t>& nibbles, size_t idx, const Value& value);
    std::optional<Value> Lookup(const NodePtr& node, const std::vector<uint8_t>& nibbles, size_t idx) const;
