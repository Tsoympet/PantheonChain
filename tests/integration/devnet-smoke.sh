...
for port in 18332 19332 20332; do
  curl -fsS "http://127.0.0.1:${port}/health" | rg -q '"status"\s*:\s*"ok"'
  curl -fsS "http://127.0.0.1:${port}/chain/info" | rg -q '"layer"'
...
rpc_call 19332 transfers/get "[\"${L2_TXID}\"]" | rg -q '"found"\s*:\s*true'
...
rpc_call 20332 evm/call "[{\"address\":\"${L3_ADDR}\",\"data\":\"0xabcdef01\"}]" | rg -q '"ok"\s*:\s*true'

# Anchoring verification OBOLOS -> DRACHMA -> TALANTON
L2_COMMIT=$(rpc_call 19332 commitments/get '["l2-anchor-1"]')
echo "$L2_COMMIT" | rg -q '"source_chain"\s*:\s*"OBOLOS"'
...
echo "$L1_COMMIT" | rg -q '"source_chain"\s*:\s*"DRACHMA"'
echo "$L1_COMMIT" | rg -q "$OB_HASH"
