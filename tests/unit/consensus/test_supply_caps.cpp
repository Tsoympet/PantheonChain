...
void TestTalanton21MCapNeverExceeded() {
    ...
    std::cout << "  Final supply: " << accumulated_supply / AssetSupply::BASE_UNIT << " TALN"
              << std::endl;
    assert(accumulated_supply <= MAX_SUPPLY);
...
void TestDrachma41MCapNeverExceeded() {
    ...
    std::cout << "  Final supply: " << accumulated_supply / AssetSupply::BASE_UNIT << " DRM"
              << std::endl;
    assert(accumulated_supply <= MAX_SUPPLY);
...
void TestObolos61MCapNeverExceeded() {
    ...
    std::cout << "  Final supply: " << accumulated_supply / AssetSupply::BASE_UNIT << " OBL"
              << std::endl;
    assert(accumulated_supply <= MAX_SUPPLY);
...
void TestSupplyCapEnforcementAtEveryHeight() {
    ...
    std::vector<uint64_t> test_heights = {0,
                                          1,
                                          100,
                                          1000,
                                          10000,
...
void TestAsymptoticSupplyApproach() {
    ...
    for (const auto& asset : assets) {
        ...
        for (int exponent = 1; exponent <= 6; exponent++) {
            uint64_t height =
                static_cast<uint64_t>(std::pow(10, exponent)) * Issuance::HALVING_INTERVAL;
            uint64_t supply = Issuance::CalculateSupplyAtHeight(height, asset.id);
            ...
            if (exponent == 6) {
                std::cout << "    At height " << height << ": " << supply / AssetSupply::BASE_UNIT
                          << " " << asset.ticker << " (" << percentage << "% of max)" << std::endl;
            }
...
int main() {
    ...
    std::cout
        << "Mathematical proof: Supply caps (21M TALN / 41M DRM / 61M OBL) are never exceeded."
        << std::endl;
