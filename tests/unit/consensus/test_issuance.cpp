...
void TestHalvingSchedule() {
    ...
    std::cout << "  Height " << height_1 << ": " << reward_1 / AssetSupply::BASE_UNIT << " TALN"
              << std::endl;
    std::cout << "  Height " << height_2 << ": " << reward_2 / AssetSupply::BASE_UNIT << " TALN"
              << std::endl;
...
void TestSupplyCalculation() {
    ...
    uint64_t supply_halving =
        Issuance::CalculateSupplyAtHeight(Issuance::HALVING_INTERVAL, AssetID::TALANTON);
    uint64_t expected_supply = Issuance::HALVING_INTERVAL * reward_0;
    assert(supply_halving == expected_supply);

    std::cout << "  Supply after " << Issuance::HALVING_INTERVAL
              << " blocks: " << supply_halving / AssetSupply::BASE_UNIT << " TALN" << std::endl;
...
void TestSupplyNeverExceedsCap() {
    ...
    for (AssetID asset : assets) {
        ...
        std::cout << "  " << AssetSupply::GetAssetTicker(asset)
                  << " max supply: " << max_supply / AssetSupply::BASE_UNIT << " (OK)" << std::endl;
    }
...
void TestSupplyAtMaxHeight() {
    ...
    std::cout << "  Supply at height " << max_height << ": " << supply / AssetSupply::BASE_UNIT
              << " TALN" << std::endl;
