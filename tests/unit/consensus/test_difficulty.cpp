...
void TestDifficultyAdjustment() {
    ...
    uint32_t new_bits =
        Difficulty::CalculateNextDifficulty(current_bits, expected_time, expected_time);
    assert(new_bits == current_bits);

    // Test 2: Blocks found too quickly (double difficulty)
    uint32_t half_time = expected_time / 2;
    new_bits = Difficulty::CalculateNextDifficulty(current_bits, half_time, expected_time);
    ...
    new_bits = Difficulty::CalculateNextDifficulty(current_bits, double_time, expected_time);
...
void TestTimewarpProtection() {
    ...
    uint32_t new_bits_min =
        Difficulty::CalculateNextDifficulty(current_bits, Difficulty::MIN_TIMESPAN, expected_time);
    ...
    uint32_t new_bits_max =
        Difficulty::CalculateNextDifficulty(current_bits, Difficulty::MAX_TIMESPAN, expected_time);
