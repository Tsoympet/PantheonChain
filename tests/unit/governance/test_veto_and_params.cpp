...
static std::vector<uint8_t> MakeSignature(const Schnorr::PrivateKey &priv, uint64_t proposal_id,
                                          const std::vector<uint8_t> &voter_pub, VoteChoice choice,
...
[[maybe_unused]] static bool CastVote(VotingSystem &vs, uint64_t proposal_id, uint8_t key_seed,
                                      VoteChoice choice, uint64_t power) {
...
void TestVetoExactlyAtThreshold() {
    std::cout << "Test: Veto exactly AT threshold (not strictly greater) → not triggered"
              << std::endl;
...
void TestVetoThresholdUpdateWithLimits() {
    std::cout << "Test: veto_threshold_bps can be updated within constitutional limits"
              << std::endl;
