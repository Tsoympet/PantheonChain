...
struct RingSignature {
    ...
    bool IsValid() const { return !signatures.empty() && !ring.empty(); }
};
...
class RingSigner {
  public:
     ...
     */
    static RingSignature Sign(const std::vector<uint8_t>& message,
     ...
     */
    static std::array<uint8_t, 32> GenerateKeyImage(const std::array<uint8_t, 32>& secret_key,
                                                    const std::array<uint8_t, 33>& public_key);

  private:
    static std::array<uint8_t, 32> HashToPoint(const std::array<uint8_t, 33>& public_key);
};
...
class RingVerifier {
  public:
     ...
     */
    static bool Verify(const RingSignature& signature, const std::vector<uint8_t>& message);
     ...
     */
    static bool
    CheckKeyImageUniqueness(const std::array<uint8_t, 32>& key_image,
...
class LinkableRingSignature {
  public:
     ...
     */
    static RingSignature SignLinkable(const std::vector<uint8_t>& message,
                                      const std::vector<std::array<uint8_t, 33>>& ring_keys,
     ...
     */
    static bool VerifyLinkable(const RingSignature& signature, const std::vector<uint8_t>& message);

    /**
     * Check if two signatures are from same signer
     */
    static bool AreLinked(const RingSignature& sig1, const RingSignature& sig2);
};
...
class StealthAddress {
  public:
     ...
     */
    static std::array<uint8_t, 33> Generate(const std::array<uint8_t, 33>& view_key,
     ...
     */
    static bool BelongsTo(const std::array<uint8_t, 33>& stealth_addr,
     ...
     */
    static std::array<uint8_t, 32> RecoverSecretKey(const std::array<uint8_t, 32>& view_secret,
