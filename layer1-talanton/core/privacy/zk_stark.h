...
class STARKProver {
  public:
     ...
     */
    STARKProof ProveStatement(const std::vector<uint8_t>& witness,
  ...
  private:
    ...
    BuildMerkleTree(const std::vector<std::vector<uint64_t>>& trace);

    std::vector<uint8_t> FRIProtocol(const std::vector<std::vector<uint64_t>>& polynomial);
};
...
class STARKVerifier {
  public:
     ...
     * Verify STARK proof
     */
    bool VerifyProof(const STARKProof& proof, const std::vector<uint8_t>& public_input,
