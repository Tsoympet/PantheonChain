...
std::array<uint8_t, 32> Difficulty::CompactToBits256(uint32_t compact) {
    ...
    // This represents: 0x00ffff * 256^(29-3) = 0x00ffff at byte position 26,27,28 (0-indexed,
    // little-endian) In big-endian representation: 0x00000000ffff00000000... In little-endian byte
    // array (index 0 is least significant): bytes[28]=0xff, bytes[27]=0xff, bytes[26]=0x00
    ...
    if (exponent <= 3) {
        ...
        if (exponent >= 2)
            target[1] = static_cast<uint8_t>(shifted >> 8);
        if (exponent >= 3)
            target[2] = static_cast<uint8_t>(shifted >> 16);
    } else if (exponent <= 32) {
...
uint32_t Difficulty::CalculateNextDifficulty(uint32_t current_bits, uint32_t time_span,
