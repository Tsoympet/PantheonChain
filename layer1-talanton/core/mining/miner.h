...
class Miner {
  public:
     ...
     */
    std::optional<primitives::Block> MineBlock(const BlockTemplate& block_template,
  ...
  private:
    ...
    CreateCoinbaseTransaction(uint32_t height, const std::vector<primitives::AssetAmount>& rewards);
    ...
    CalculateFees(const std::vector<primitives::Transaction>& transactions);
    ...
    ComputeMerkleRoot(const std::vector<primitives::Transaction>& transactions);
