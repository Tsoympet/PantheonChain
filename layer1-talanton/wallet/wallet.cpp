...
Wallet::Wallet(const std::array<uint8_t, 32>& seed) : seed_(seed), next_index_(0) {}
...
Wallet::CreateTransaction(const std::vector<primitives::TxOutput>& outputs,
                          primitives::AssetID asset, uint64_t fee_amount) {
    ...
    for (size_t i = 0; i < tx.inputs.size(); i++) {
        ...
        tx.inputs[i].signature_script =
            std::vector<uint8_t>(signature_opt->begin(), signature_opt->end());
    }

    return tx;
}

void Wallet::AddUTXO(const primitives::OutPoint& outpoint, const primitives::TxOutput& output,
                     uint32_t height) {
    WalletUTXO utxo;
