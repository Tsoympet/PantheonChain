      ...
      rpcClient(rpc)
{
    ...
    if (rpcClient) {
        connect(rpcClient, &RPCClient::transactionHistoryUpdated,
                this, &TransactionPage::onTransactionHistoryUpdated);
    }
}
...
void TransactionPage::setupUI() {
    ...
    filterComboBox->addItems({tr("All"), tr("Sent"), tr("Received"), 
                              tr("TALN Only"), tr("DRM Only"), tr("OBL Only")});
    connect(filterComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &TransactionPage::onFilterChanged);
    ...
    transactionTable->setHorizontalHeaderLabels({
        tr("Date/Time"), tr("Type"), tr("Asset"), 
        tr("Amount"), tr("Address"), tr("TXID")
    });
...
void TransactionPage::loadTransactions() {
    ...
    QList<Transaction> sampleTxs = {
        {QDateTime::currentDateTime().addDays(-1).toString("yyyy-MM-dd hh:mm:ss"),
         "Received", "TALN", 100.50000000, "parthenon1q123...", "abc123..."},
        {QDateTime::currentDateTime().addDays(-2).toString("yyyy-MM-dd hh:mm:ss"),
         "Sent", "DRM", -50.25000000, "parthenon1q456...", "def456..."},
        {QDateTime::currentDateTime().addDays(-3).toString("yyyy-MM-dd hh:mm:ss"),
         "Received", "OBL", 200.00000000, "parthenon1q789...", "ghi789..."},
    };
    ...
    for (const auto &tx : sampleTxs) {
        ...
        transactionTable->setItem(row, 3, new QTableWidgetItem(
            QString::number(tx.amount, 'f', 8)));
