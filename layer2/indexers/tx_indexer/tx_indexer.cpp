...
void TxIndexer::IndexTransaction(const primitives::Transaction& tx, uint32_t height, uint32_t block_time) {
    impl_->IndexTransaction(tx, height, block_time);
}

std::vector<primitives::Transaction> TxIndexer::GetTransactionsByAddress(
    const std::vector<uint8_t>& address,
    uint32_t limit
) {
...
std::optional<primitives::Transaction> TxIndexer::GetTransactionById(
    const std::array<uint8_t, 32>& txid
) {
