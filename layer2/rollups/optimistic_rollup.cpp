...
std::vector<uint8_t> OptimisticRollup::CompressBatch(const RollupBatch& batch) const
{
    ...
    compressed.insert(compressed.end(), 
                     batch.state_root_before.begin(), 
                     batch.state_root_before.end());
    compressed.insert(compressed.end(), 
                     batch.state_root_after.begin(), 
...
std::optional<RollupBatch> OptimisticRollup::DecompressBatch(
...
std::array<uint8_t, 32> RollupSequencer::CalculateStateRoot(
...
RollupVerifier::RollupVerifier(OptimisticRollup* rollup)
    : rollup_(rollup)
{
}
...
std::array<uint8_t, 32> RollupVerifier::ReExecuteTransaction(
    const RollupTx& tx,
