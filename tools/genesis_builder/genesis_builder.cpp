...
primitives::Block CreateGenesisBlock(const GenesisConfig& config) {
    ...
    if (!config.premine_addresses.empty()) {
        ...
        uint64_t tal_per_address = 1000000 * 100000000;  // 1M TALANTON each
        uint64_t dra_per_address = 2000000 * 100000000;  // 2M DRACHMA each
        uint64_t obl_per_address = 3000000 * 100000000;  // 3M OBOLOS each
        ...
        for (const auto& addr_hex : config.premine_addresses) {
            ...
            tal_output.pubkey_script = std::vector<uint8_t>(32, 0);  // Dummy
            ...
            dra_output.pubkey_script = std::vector<uint8_t>(32, 0);  // Dummy
            ...
            obl_output.pubkey_script = std::vector<uint8_t>(32, 0);  // Dummy
