...
std::string EncodeCommitment(const Commitment& commitment) {
    ...
    for (std::size_t i = 0; i < commitment.signatures.size(); ++i) {
        ...
        stream << signature.validator_id << '|' << signature.stake << '|'
               << signature.signature;
    }
    return stream.str();
}
...
CommitmentValidationResult DecodeCommitment(const std::string& encoded,
    ...
    while (cursor <= fields[7].size()) {
        ...
        if (!item.empty()) {
            ...
            try {
                signature.stake = std::stoull(item.substr(first + 1, second - first - 1));
            } catch (const std::exception&) {
                return {false, "validator stake must be an unsigned integer"};
            }
